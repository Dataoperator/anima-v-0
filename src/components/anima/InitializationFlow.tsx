...
import { Brain, Zap, Stars, Heart, Sparkles, ArrowRight } from 'lucide-react';
...
const InitializationFlow: React.FC<InitializationFlowProps> = ({ onInitialized }) => {
  ...
  const { initializeQuantumState } = useIC();
  ...
  const handleInitialization = async () => {
    ...
    try {
      ...
      await initializeQuantumState(principal, initialState);
      setInitialized(true);
      onInitialized();
    } catch (err) {
      console.error('Initialization error:', err);
      setError(err instanceof Error ? err.message : 'Failed to initialize quantum state');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <motion.div 
        className="min-h-screen bg-anima-gradient flex items-center justify-center"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      >
        <motion.div
      ...
      <ErrorBoundary>
        <div className="min-h-screen bg-anima-gradient overflow-hidden">
          ...
          <div className="relative container mx-auto px-4 py-16 flex flex-col items-center">
            ...
            >
              <AnimaCreationForm 
                onSuccess={handleInitialization}
                isDisabled={loading} 
              />
            </motion.div>
